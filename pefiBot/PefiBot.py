...
class PefiBot:
    ...
    async def suggest(self, ctx):
        ...
        if not ctx.message.guild.id in self.channels.suggestion_channel:
            print("Channel " + Constants.PEFISUGGEST_CHANNEL_NAME + " not found in " + ctx.message.guild)
            return
    ...
    async def on_raw_reaction_add(self, payload):
        """Add pefi role when a reaction is added on a particular message (not a message from pefibot or a
        reaction of pefibot) """
        if payload.user_id == self.bot.user.id or payload.message_id == self.bot.user.id:  # check if user that reacted is not pefiBot and that the message isn't from pefiBot
            return
